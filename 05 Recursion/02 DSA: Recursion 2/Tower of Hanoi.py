...
# https://www.scaler.com/academy/mentee-dashboard/class/236076/homework/problems/15010/?navref=cl_pb_nv_tb

class Solution:
    # @param A : integer
    # @return a list of list of integers
    def tower_of_hanoi(self, n, source, destination, auxiliary, moves):
        if n == 1:
            moves.append([1, source, destination])
            return
        
        # Move n-1 disks from source to auxiliary
        self.tower_of_hanoi(n - 1, source, auxiliary, destination, moves)
        
        # Move the nth disk from source to destination
        moves.append([n, source, destination])  # Store the move
        
        # Move the n-1 disks from auxiliary to destination
        self.tower_of_hanoi(n - 1, auxiliary, destination, source, moves)

    def towerOfHanoi(self, A):
        moves = []  
        self.tower_of_hanoi(A, 1,3,2, moves)
        return moves