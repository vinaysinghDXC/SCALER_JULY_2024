// https://www.scaler.com/academy/mentee-dashboard/class/297873/homework/problems/6284?navref=cl_tt_lst_sl
...
int solve(vector<int> &A, vector<int> &B) {
    ...
    vector<pair<int, char>> roads; // {cost, type}, 'R' for row, 'C' for column
    
    for (int i = 0; i < N; ++i) roads.push_back({A[i], 'R'});
    for (int j = 0; j < M; ++j) roads.push_back({B[j], 'C'});
    
    sort(roads.begin(), roads.end());

    long long totalCost = 0;
    long long rowsRemaining = N + 1, colsRemaining = M + 1;

    for (auto& road : roads) {
        if (road.second == 'R') {
            totalCost = (totalCost + road.first * colsRemaining) % MOD;
            rowsRemaining--;
        } else {
            totalCost = (totalCost + road.first * rowsRemaining) % MOD;
            colsRemaining--;
        }
    }
